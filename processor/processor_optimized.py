...
class FootScanProcessor:
    ...
    def process_scan(self, input_dir: str, output_dir: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Generate diagnostic PDF report (always try, even if there are errors)
            try:
                logger.info(f"[Scan-{process_id}] Generating diagnostic PDF report")
                
                # Check if we have visualization paths
                if "visualizations" in results:
                    visualizations = results["visualizations"]
                else:
                    # Create default empty dict if no visualizations
                    visualizations = {}
                
                # Add 3D model thumbnail if available
                if os.path.exists(os.path.join(output_dir, "3d_models", f"thumbnail_{process_id}.jpg")):
                ...
                if report_path and os.path.exists(report_path):
                    ...
                    results["pdf_report_path"] = report_path
                else:
